...
export class ProcurementService {
  ...
  async createGrn(data: {
    ...
    await this.prisma.sql(
      ...
      [grnId, data.grnNumber, data.purchaseOrderId || null, data.vendorId, data.storeId, data.description || null, 'COMPLETED'],
    );
    ...
    for (const line of data.lines) {
      ...
      if (line.batchNumber) {
        ...
        const batchRows = await this.prisma.sql(
          ...
          [bid, line.skuId, line.batchNumber, mfg, exp, data.storeId, data.vendorId, line.receivedQty, line.unitCost, line.receivedQty * line.unitCost],
        );
        batchId = bid;
      }
      ...
      await this.prisma.sql(
        ...
        [lid, grnId, line.skuId, batchId, line.orderedQty, line.receivedQty, line.rejectedQty || 0, line.unitCost, line.taxAmount || 0],
      );
