...
export class BillingService {
  ...
  async createInvoice(
    orderId: string,
    data: { invoiceNumber: string; dueDate?: Date },
  ) {
    ...
    const lines = await this.prisma.sql(
    ...
    const invoiceRows = await this.prisma.sql(
      'SELECT * FROM "Invoice" WHERE "id" = $1 LIMIT 1',
      [invoiceId],
    );
    const invoice = invoiceRows[0];
    invoice.lines = await this.prisma.sql(
  ...
  async getInvoice(invoiceId: string) {
    const invoices = await this.prisma.sql(
      'SELECT * FROM "Invoice" WHERE "id" = $1 LIMIT 1',
      [invoiceId],
    );
    ...
    invoice.payments = await this.prisma.sql(
      'SELECT * FROM "Payment" WHERE "invoiceId" = $1',
      [invoiceId],
    );
    return invoice;
  }
  ...
  async getInvoices(storeId: string) {
    const invoices = await this.prisma.sql(
      'SELECT * FROM "Invoice" WHERE "storeId" = $1',
      [storeId],
    );
    for (const inv of invoices) {
      inv.lines = await this.prisma.sql(
        'SELECT * FROM "InvoiceLine" WHERE "invoiceId" = $1',
        [inv.id],
      );
    }
