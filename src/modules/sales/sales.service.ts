...
export class SalesService {
  ...
  async createSalesOrder(data: {
    ...
    await this.prisma.sql(
      ...
      [orderId, data.orderNumber, data.storeId, data.customerId || null, 'COMPLETED', totalAmount, totalTaxAmount, totalDiscountAmount, netAmount],
    );
    ...
    for (const item of lineItemsData) {
      ...
      await this.prisma.sql(
        ...
        [lineId, orderId, item.skuId, item.batchId, item.quantity, item.unitPrice, item.discountAmount, item.taxRate, item.taxAmount, item.netAmount],
      );
    }
