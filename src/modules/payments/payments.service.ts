...
export class PaymentsService {
  ...
  async processPayment(data: {
    ...
    const payments = await this.prisma.sql(
      'SELECT * FROM "Payment" WHERE "invoiceId" = $1 AND "status" = $2',
      [data.invoiceId, 'SUCCESS'],
    );
    ...
    if (data.amount > remainingAmount) {
      throw new BadRequestException(
        `Payment amount exceeds outstanding balance. Outstanding: ${remainingAmount}`,
      );
    }
